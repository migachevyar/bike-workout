...
// Отступ сверху: 60px шапка Telegram + выемка iPhone
// Не вызываем requestFullscreen — тогда шапка TG всегда видна и 60px всегда нужны
const SAFE_TOP    = "calc(60px + env(safe-area-inset-top, 0px))";
...
// ─── PROGRAMS ─────────────────────────────────────────────────────────────────
const PROGS = [
  {id:"b1",level:"beginner", ll:"Новичок", goal:"fat",       gl:"Жиросжигание", name:"Старт: сжигание жира",   dur:25, desc:"Мягкий вход. Пульс 60–70% от макс — идеальная зона жиросжигания.", iv:[{id:"b1a",t:"slow",d:300},{id:"b1b",t:"medium",d:120},{id:"b1c",t:"slow",d:180},{id:"b1d",t:"medium",d:120},{id:"b1e",t:"slow",d:180},{id:"b1f",t:"medium",d:120},{id:"b1g",t:"slow",d:480}]},
  ...
  {id:"i1",level:"intermediate",ll:"Опытный",goal:"fat",     gl:"Жиросжигание", name:"HIIT: жиросжигание",      dur:30, desc:"Спринты чередуются с восстановлением. Метаболизм ускоряется на 24–48ч.", iv:[{id:"i1w",t:"slow",d:300},...Array.from({length:7},(_,i)=>[{id:`i1f${i}`,t:"fast",d:30},{id:`i1r${i}`,t:"slow",d:90}]).flat(),{id:"i1c",t:"slow",d:300}]},
...
const TIMER_KEY = "bw_timer_v2";
const saveTimer  = s => localStorage.setItem(TIMER_KEY, JSON.stringify(s));
...
// ─── DESIGN ───────────────────────────────────────────────────────────────────
const C = {
  bg:"#09090f", surface:"rgba(255,255,255,0.05)", border:"rgba(255,255,255,0.08)",
  text:"#fff", sub:"rgba(255,255,255,0.45)", muted:"rgba(255,255,255,0.2)",
  green:"#4ade80", cyan:"#22d3ee", yellow:"#facc15", red:"#c0392b",
...
const LVL_LABEL = {beginner:"Новичок", intermediate:"Опытный", pro:"Профи"};
...
// ─── BASE COMPONENTS ──────────────────────────────────────────────────────────
// Страница с учётом отступов навигации
function Page({children, noBottomPad=false, style={}}) {
  return (
    <div style={{
      minHeight:"100vh", background:C.bg, color:C.text, boxSizing:"border-box",
      paddingTop: SAFE_TOP,
      paddingLeft:16, paddingRight:16,
      paddingBottom: noBottomPad ? 0 : `calc(${NAV_H}px + ${SAFE_BOTTOM} + 16px)`,
...
function Loader() {
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}><div style={{fontSize:36}}>🚴</div><div style={{color:C.muted,fontSize:14,marginTop:8}}>Загрузка...</div></div>
    </div>
  );
}
...
const IconBtn = ({onClick,children,style={}}) => {
  ...
  return (
    <button onClick={onClick} {...ph}
      style={{width:38,height:38,borderRadius:"50%",background:C.surface,border:`1px solid ${C.border}`,
        display:"flex",alignItems:"center",justifyContent:"center",color:C.text,
        cursor:"pointer",flexShrink:0,...ps,...style}}>
      {children}
    </button>
  );
};

const GreenBtn = ({onClick,children,disabled=false,style={},...rest}) => {
  const [ph,ps] = usePress(0.97);
  return (
    <button onClick={onClick} disabled={disabled} {...ph} {...rest}
      style={{width:"100%",background:disabled?"rgba(255,255,255,0.07)":"linear-gradient(135deg,#4ade80,#22d3ee)",
        border:"none",borderRadius:14,padding:"15px",color:disabled?"rgba(255,255,255,0.3)":"#000",
        fontSize:15,fontWeight:700,cursor:disabled?"default":"pointer",
        boxShadow:disabled?"none":"0 4px 20px rgba(74,222,128,0.2)",...ps,...style}}>
      {children}
    </button>
  );
};
...
function IvBar({intervals=[], h=5}) {
  ...
  return (
    <div style={{display:"flex",gap:2,height:h,borderRadius:h/2,overflow:"hidden"}}>
      {intervals.map((iv,i) => (
        <div key={i} style={{flex:iv.d||iv.duration||1,background:IV[iv.t||iv.type]?.color||C.green,minWidth:3,transition:"flex 0.3s"}}/>
      ))}
    </div>
  );
}
...
// ─── BOTTOM NAVIGATION ────────────────────────────────────────────────────────
const NAV_ITEMS = [
  {id:"start",    label:"Старт",    Icon:I.Star},
  {id:"programs", label:"Программы",Icon:I.Book},
        ...
        return (
          <button key={item.id} onClick={() => onSelect(item.id)}
            style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
              ...
              transition:"color 0.2s",paddingBottom:0}}>
            <item.Icon size={22} style={{color:"inherit",strokeWidth:isActive?2.5:1.8}}/>
            <span style={{fontSize:10,fontWeight:isActive?600:400,letterSpacing:"0.02em"}}>{item.label}</span>
          </button>
        );
      })}
    </div>
  ...
  return (
    <div style={{position:"relative",flex:1,height:ITEM_H*5,overflow:"hidden",borderRadius:12}}>
        ...
        {Array.from({length:max+1},(_,v) => (
          <div key={v} onClick={() => { onChange(v); scrollTo(v); }}
            style={{height:ITEM_H,display:"flex",alignItems:"center",justifyContent:"center",
              fontSize:26,fontWeight:v===value?600:300,
              color:v===value?"#fff":"rgba(255,255,255,0.22)",
              fontVariantNumeric:"tabular-nums",cursor:"pointer",userSelect:"none",transition:"color 0.12s"}}>
      ...
      <div style={{position:"absolute",inset:0,pointerEvents:"none",
        background:`linear-gradient(to bottom, ${C.bg} 0%, transparent 32%, transparent 68%, ${C.bg} 100%)`,zIndex:2}}/>
      <div style={{position:"absolute",left:8,right:8,top:ITEM_H*2,height:ITEM_H,
        border:`1px solid rgba(74,222,128,0.25)`,borderRadius:8,zIndex:1,pointerEvents:"none"}}/>
    </div>
  );
}
  ...
  return createPortal(
    <div style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()}
        style={{width:"100%",background:"#141420",borderRadius:"20px 20px 0 0",
          padding:"16px 20px",paddingBottom:`max(24px, ${SAFE_BOTTOM})`,animation:"slideUp 0.25s ease"}}>
        <div style={{fontSize:16,fontWeight:500,marginBottom:16,textAlign:"center"}}>Длительность интервала</div>
        <div style={{display:"flex",alignItems:"center",gap:0}}>
          <WheelColumn value={m} max={99} onChange={setM}/>
          <div style={{fontSize:24,color:C.muted,padding:"0 10px",flexShrink:0}}>:</div>
          <WheelColumn value={s} max={59} onChange={setS}/>
        </div>
        <div style={{display:"flex",gap:8,marginTop:6,marginBottom:14}}>
          <div style={{flex:1,textAlign:"center",color:C.sub,fontSize:12}}>{m} мин</div>
          <div style={{flex:1,textAlign:"center",color:C.sub,fontSize:12}}>{s} сек</div>
        </div>
        <GreenBtn onClick={() => { onChange(m*60+s); onClose(); }}>Готово</GreenBtn>
      </div>
    </div>,
    document.body
  );
...
// ─── INTERVAL ROW (swipe to delete, drag to reorder) ─────────────────────────
const SLIDE = 78;
...
function IvRow({iv, index, onChange, onDelete, dragHandle}) {
  ...
  const isH = useRef(false), dragging = useRef(false);

  const onTS = e => {
    if (e.target.closest("[data-drag]")) return;
    startX.current=e.touches[0].clientX; startY.current=e.touches[0].clientY;
    isH.current=false; dragging.current=false;
  };
  const onTM = e => {
    if (e.target.closest("[data-drag]")||startX.current===null) return;
    const dx=e.touches[0].clientX-startX.current, dy=e.touches[0].clientY-startY.current;
    if (!dragging.current) {
      if (Math.abs(dx)<4&&Math.abs(dy)<4) return;
      isH.current=Math.abs(dx)>Math.abs(dy); dragging.current=true;
    }
  ...
  const onTE = () => {
    if (!dragging.current||!isH.current){startX.current=null;return;}
  ...
  return (
    <div style={{position:"relative",marginBottom:8,borderRadius:14,overflow:"hidden"}}>
      <div style={{position:"absolute",right:0,top:0,bottom:0,width:SLIDE-8,background:"#6b1515",
        borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",
        flexDirection:"column",gap:3,opacity:open?1:0,transition:"opacity 0.2s",zIndex:0}}>
        <button onClick={()=>{setOpen(false);setTx(0);setTimeout(onDelete,180)}}
          style={{background:"none",border:"none",color:"#fff",cursor:"pointer",
            display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
          <I.Trash size={18}/><span style={{fontSize:10,fontWeight:700}}>УДАЛИТЬ</span>
        </button>
      </div>
      <div onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}
        style={{background:"rgba(255,255,255,0.05)",border:`1px solid ${cfg.color}22`,borderRadius:14,
          padding:"10px 12px",display:"flex",alignItems:"center",gap:8,
          transform:`translateX(${tx}px)`,
          transition:(dragging.current&&isH.current)?"none":"transform 0.3s cubic-bezier(0.25,0.46,0.45,0.94)",
          position:"relative",zIndex:1,userSelect:"none",touchAction:open?"none":"pan-y",willChange:"transform"}}>
        {/* Drag handle */}
        <div data-drag="1" {...dragHandle}
          style={{width:28,height:28,borderRadius:8,background:"rgba(255,255,255,0.07)",
            border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",
            color:C.muted,fontSize:13,fontWeight:600,cursor:"grab",flexShrink:0,touchAction:"none"}}>
          {index+1}
        </div>
        {/* Type */}
        <div style={{display:"flex",background:"rgba(255,255,255,0.06)",borderRadius:11,padding:2,gap:2,flexShrink:0}}>
          {Object.entries(IV).map(([t,c]) => (
            <button key={t} onClick={() => onChange({...iv,t,type:t})}
              style={{background:(iv.t||iv.type)===t?c.color:"transparent",border:"none",borderRadius:8,
                width:32,height:32,fontSize:17,cursor:"pointer",transition:"background 0.15s",
                display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              {c.emoji}
            </button>
          ))}
        </div>
        <div style={{fontSize:12,color:cfg.color,fontWeight:600,flex:1,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>{cfg.label}</div>
        <button onClick={() => setPicker(true)}
          style={{background:"rgba(255,255,255,0.07)",border:`1px solid ${C.border}`,borderRadius:9,
            padding:"6px 10px",color:"#fff",fontSize:14,fontWeight:500,cursor:"pointer",
            flexShrink:0,fontVariantNumeric:"tabular-nums",minWidth:56,textAlign:"center"}}>
          {fmtT(dur)}
        </button>
      </div>
      {pickerOpen && <DurPicker duration={dur} onChange={d=>onChange({...iv,d,duration:d})} onClose={()=>setPicker(false)}/>}
    </div>
  );
}
...
function SortList({intervals, onChange}) {
  ...
  const startDrag = (idx, e) => {
    ...
    setDragging(idx); setOverIdx(idx); setGhostY(0);
  };
  const onMove = useCallback(e => {
    ...
    rowRefs.current.forEach((r,i) => {
      ...
      if (y > rect.top && y < rect.bottom) best = i;
    });
    setOverIdx(Math.max(0, Math.min(intervals.length-1, best)));
  }, [dragging, intervals.length]);
  ...
  const onEnd = useCallback(() => {
    ...
    setDragging(null); setOverIdx(null); setGhostY(0);
  ...
  return (
    <div>
      {display.map((origIdx, dispIdx) => {
        ...
        const isDragging = dragging===origIdx;
        const isTarget = overIdx===dispIdx && dragging!==null && dragging!==origIdx;
        return (
          <div key={iv.id} ref={el => rowRefs.current[dispIdx]=el}
            style={{transform:isDragging?`translateY(${ghostY}px) scale(1.02)`:isTarget?"translateY(3px)":"translateY(0)",
              transition:isDragging?"none":"transform 0.18s ease",
              zIndex:isDragging?10:1,position:"relative",
              boxShadow:isDragging?"0 8px 32px rgba(0,0,0,0.6)":"none",opacity:isDragging?0.9:1}}>
...
function WCard({workout, onStart, onEdit, onDelete}) {
  const total = workout.intervals?.reduce((s,i)=>s+(i.d||i.duration||0),0)||0;
  const [ph,ps] = usePress(0.97);
  ...
  const openMenu = e => {
    ...
    setMPos({top:r.bottom+6, right:window.innerWidth-r.right});
    setMenu(true);
  };
  ...
  return (
    <div style={{animation:"slideUp 0.2s ease both"}}>
      <div {...ph} onClick={onStart}
        style={{...ps,background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"14px 16px",cursor:"pointer"}}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
          <div style={{flex:1,marginRight:8}}>
            <div style={{fontSize:16,fontWeight:500,marginBottom:5}}>{workout.name}</div>
            <div style={{display:"flex",gap:12,marginBottom:10}}>
              <span style={{display:"flex",alignItems:"center",gap:4,color:C.sub,fontSize:13}}><I.Clock size={12}/>{fmtD(total)}</span>
              <span style={{display:"flex",alignItems:"center",gap:4,color:C.sub,fontSize:13}}><I.Zap size={12}/>{workout.intervals?.length||0} инт.</span>
            </div>
            <IvBar intervals={ivs}/>
          </div>
          <button ref={btnRef} onClick={openMenu}
            style={{background:"none",border:"none",color:C.muted,cursor:"pointer",padding:"2px 6px",fontSize:18,letterSpacing:2,flexShrink:0}}>···</button>
        </div>
      </div>
      {menu && createPortal(<>
        <div onClick={()=>setMenu(false)} style={{position:"fixed",inset:0,zIndex:9998}}/>
        <div style={{position:"fixed",top:mPos.top,right:mPos.right,zIndex:9999,background:"#1c1c2a",
          border:`1px solid ${C.border}`,borderRadius:14,overflow:"hidden",minWidth:170,
          boxShadow:"0 20px 48px rgba(0,0,0,0.8)",animation:"fadeIn 0.15s ease"}}>
...
function HistCard({result, onClick, onDelete}) {
  ...
  const fmt = new Date(result.completedAt).toLocaleDateString("ru-RU",{day:"numeric",month:"short"});
  const [ph,ps] = usePress(0.97);
  const [open,setOpen] = useState(false), [tx,setTx] = useState(0);
  const startX=useRef(null), startY=useRef(null), isH=useRef(false), dragging=useRef(false);

  const onTS=e=>{startX.current=e.touches[0].clientX;startY.current=e.touches[0].clientY;isH.current=false;dragging.current=false;};
  const onTM=e=>{
    if(startX.current===null) return;
    const dx=e.touches[0].clientX-startX.current,dy=e.touches[0].clientY-startY.current;
    if(!dragging.current){if(Math.abs(dx)<4&&Math.abs(dy)<4)return;isH.current=Math.abs(dx)>Math.abs(dy);dragging.current=true;}
    if(!isH.current) return;
    e.preventDefault();e.stopPropagation();
    setTx(Math.max(-SLIDE,Math.min(0,(open?-SLIDE:0)+dx)));
  };
  const onTE=()=>{if(!dragging.current||!isH.current){startX.current=null;return;}dragging.current=false;const o=tx<-SLIDE/2;setOpen(o);setTx(o?-SLIDE:0);startX.current=null;};

  return (
    <div style={{position:"relative",marginBottom:8,borderRadius:14,overflow:"hidden"}}>
      <div style={{position:"absolute",right:0,top:0,bottom:0,width:SLIDE-8,background:"#6b1515",borderRadius:14,
      ...
      <div {...ph} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}
        onClick={()=>!open&&onClick()}
        style={{...ps,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"13px 14px",
          cursor:"pointer",transform:`translateX(${tx}px)`,
          transition:(dragging.current&&isH.current)?"none":"transform 0.3s cubic-bezier(0.25,0.46,0.45,0.94)",
          position:"relative",zIndex:1,userSelect:"none",touchAction:open?"none":"pan-y"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
          <div style={{fontSize:15,fontWeight:500}}>{result.workoutName}</div>
          {done&&<I.Check size={16} style={{color:C.green,flexShrink:0}}/>}
        </div>
        <div style={{display:"flex",gap:14}}>
          <span style={{display:"flex",alignItems:"center",gap:4,color:C.sub,fontSize:13}}><I.Clock size={12}/>{fmtD(result.totalDuration)}</span>
          <span style={{fontSize:13,color:done?C.green:"#fb923c"}}>{result.completedIntervals}/{result.totalIntervals} инт.</span>
          <span style={{fontSize:12,color:C.muted,marginLeft:"auto"}}>{fmt}</span>
        </div>
      </div>
    </div>
  );
...
function PCard({prog, onUse, added}) {
  ...
  return (
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden",animation:"slideUp 0.18s ease both"}}>
      <div {...ph} onClick={()=>setO(v=>!v)} style={{...ps,padding:"14px 16px",cursor:"pointer"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:1,marginRight:12}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,flexWrap:"wrap"}}>
              <span style={{fontSize:11,fontWeight:700,letterSpacing:"0.08em",color:lc,background:`${lc}18`,borderRadius:5,padding:"2px 7px"}}>{prog.ll.toUpperCase()}</span>
            ...
            <div style={{display:"flex",gap:10}}>
              <span style={{fontSize:12,color:C.sub}}>{prog.dur} мин</span>
              <span style={{fontSize:12,color:C.sub}}>{prog.iv.length} инт.</span>
            </div>
          </div>
      ...
      {open&&(
        <div style={{padding:"0 16px 16px",borderTop:`1px solid ${C.border}`,animation:"fadeIn 0.18s ease"}}>
...
// ─── WEEKLY BAR CHART ─────────────────────────────────────────────────────────
function WeekChart({results}) {
  const days = ["Пн","Вт","Ср","Чт","Пт","Сб","Вс"];
  // Собираем минуты по дням текущей недели
  const now = new Date();
  ...
  results.forEach(r => {
    ...
    if (diff >= 0 && diff < 7) byDay[diff] += Math.round(r.totalDuration/60);
  });
        ...
        return (
          <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <div style={{
              ...
              background: hasVal
                ? (isToday ? "linear-gradient(to top,#4ade80,#22d3ee)" : "rgba(74,222,128,0.4)")
                : "rgba(255,255,255,0.06)",
              transition:"height 0.4s ease",
            }}/>
            <div style={{fontSize:10,color:isToday?C.green:C.muted,fontWeight:isToday?600:400}}>{days[i]}</div>
          </div>
        );
      })}
    </div>
...
function LevelSheet({current, onSelect, onClose}) {
  return createPortal(
    <div style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()}
        style={{width:"100%",background:"#141420",borderRadius:"20px 20px 0 0",
          padding:"20px 16px",paddingBottom:`max(28px, ${SAFE_BOTTOM})`,animation:"slideUp 0.25s ease"}}>
        <div style={{fontSize:17,fontWeight:600,marginBottom:16,textAlign:"center"}}>Выберите уровень</div>
        {[
          {v:"beginner",    label:"Новичок", desc:"Первые тренировки, безопасная нагрузка", emoji:"🌱"},
          {v:"intermediate",label:"Опытный", desc:"HIIT, пирамиды, 2–3 раза в неделю",    emoji:"⚡"},
          {v:"pro",         label:"Профи",   desc:"Табата, VO2max — максимальная нагрузка", emoji:"🏆"},
        ].map(l => {
          const [ph,ps]=usePress(0.97);
          const isActive = l.v===current;
          return (
            <button key={l.v} {...ph} onClick={()=>{onSelect(l.v);onClose();}}
              style={{...ps,width:"100%",background:isActive?"rgba(74,222,128,0.08)":C.surface,
                border:`1px solid ${isActive?"rgba(74,222,128,0.3)":C.border}`,borderRadius:14,
                padding:"14px",display:"flex",alignItems:"center",gap:12,textAlign:"left",
                cursor:"pointer",marginBottom:8}}>
              <div style={{width:42,height:42,borderRadius:12,background:`${LVC[l.v]}14`,
                display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:22}}>{l.emoji}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:15,fontWeight:600,color:isActive?C.green:"#fff",marginBottom:3}}>{l.label}</div>
                <div style={{fontSize:12,color:C.sub}}>{l.desc}</div>
              </div>
...
function StartPage({navigate}) {
  ...
  const [userLevel,setUL]  = useState(()=>localStorage.getItem("bw_user_level")||"");
  const [cph,cps] = usePress(0.97);

  const load = useCallback(async () => {
    const [ws,rs,pr] = await Promise.all([db.getW(), db.getR(), db.getP()]);
    setWS(ws);
    setHist([...rs].sort((a,b)=>new Date(b.completedAt)-new Date(a.completedAt)));
    setPr(pr);
    setLoad(false);
  },[]);
  ...
  const recoProgs   = userLevel ? PROGS.filter(p=>p.level===userLevel) : PROGS.slice(0,3);
  ...
  return (
    <Page>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:22}}>
        <div style={{fontSize:28,fontWeight:300}}>{firstName?`Привет,\u00A0${firstName}!`:"Тренировки"}</div>
      </div>

      {/* Создать */}
      <button onClick={()=>navigate("create")} {...cph}
        style={{...cps,width:"100%",background:"linear-gradient(135deg,#4ade80,#22d3ee)",border:"none",
          borderRadius:14,padding:"15px 18px",cursor:"pointer",marginBottom:24,
          display:"flex",alignItems:"center",gap:12,boxShadow:"0 6px 20px rgba(74,222,128,0.2)"}}>
        <div style={{width:30,height:30,background:"rgba(0,0,0,0.15)",borderRadius:8,
          display:"flex",alignItems:"center",justifyContent:"center"}}>
          <I.Plus size={17} style={{color:"#000"}}/>
        </div>
      ...
      {/* Мои тренировки */}
      {myWorkouts.length>0&&<>
        <div style={{fontSize:17,fontWeight:600,marginBottom:12}}>Мои тренировки</div>
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
          {myWorkouts.map(w=>(
            <WCard key={w.id} workout={w}
              onStart={()=>navigate("workout",w.id)}
              onEdit={()=>navigate("edit",w.id)}
              onDelete={()=>delW(w.id)}/>
          ))}
        </div>
      </>}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div style={{fontSize:17,fontWeight:600}}>Рекомендации</div>
        {userLevel&&(
          <span style={{fontSize:12,color:LVC[userLevel],background:`${LVC[userLevel]}15`,
      ...
      {!userLevel ? (
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px",marginBottom:24}}>
          <div style={{fontSize:14,color:C.sub,marginBottom:12,lineHeight:1.5}}>Выберите уровень, чтобы получить подходящие программы</div>
          <div style={{display:"flex",gap:8}}>
            {[{v:"beginner",l:"🌱 Новичок"},{v:"intermediate",l:"⚡ Опытный"},{v:"pro",l:"🏆 Профи"}].map(({v,l})=>{
              const [ph,ps]=usePress(0.96);
              return (
                <button key={v} {...ph} onClick={()=>{localStorage.setItem("bw_user_level",v);setUL(v);}}
                  style={{...ps,flex:1,background:"rgba(255,255,255,0.06)",border:`1px solid ${C.border}`,
                    borderRadius:10,padding:"9px 4px",color:C.text,fontSize:11,fontWeight:500,cursor:"pointer",textAlign:"center"}}>
                  {l}
                </button>
              );
            })}
        ...
        <div style={{display:"flex",gap:10,overflowX:"auto",paddingBottom:6,marginBottom:24,scrollbarWidth:"none"}}>
          {recoProgs.map(p => (
            <div key={p.id} onClick={()=>startProg(p)}
              style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,
                padding:"14px",cursor:"pointer",flexShrink:0,width:185,animation:"slideUp 0.2s ease both"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                <span style={{fontSize:10,fontWeight:700,color:LVC[p.level],background:`${LVC[p.level]}18`,
                  borderRadius:5,padding:"2px 7px",letterSpacing:"0.07em"}}>{p.ll.toUpperCase()}</span>
                <span style={{fontSize:11,color:C.muted}}>{p.dur} мин</span>
                {addedIds.has(p.id)&&<span style={{fontSize:10,color:C.green}}>✓</span>}
              </div>
              <div style={{fontSize:14,fontWeight:500,marginBottom:10,lineHeight:1.3}}>{p.name}</div>
              <IvBar intervals={p.iv.map(iv=>({...iv,t:iv.t,d:iv.d}))} h={4}/>
            </div>
          ))}
        </div>
      )}
      ...
      {/* История */}
      {history.length>0&&<>
        <div style={{fontSize:17,fontWeight:600,marginBottom:12}}>Недавние</div>
        <div style={{display:"flex",flexDirection:"column"}}>
          {history.slice(0,4).map(r=>(
            <HistCard key={r.id} result={r}
              onClick={()=>navigate("details",r.id)}
              onDelete={()=>delR(r.id)}/>
          ))}
          {history.length>4&&(
            <div style={{textAlign:"center",color:C.muted,fontSize:13,padding:"8px 0",cursor:"pointer"}}
              onClick={()=>{}}>
              ещё {history.length-4} тренировок
            </div>
          )}
        </div>
      </>}
...
function ProgramsPage({navigate}) {
  ...
  useEffect(()=>{(async()=>{ const ws=await db.getW(); setWS(ws); setLoad(false); })();},[]);
  ...
  return (
    <Page>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {filtered.map(p=>(
          <PCard key={p.id} prog={p} onUse={startProg} added={addedIds.has(p.id)}/>
        ))}
      </div>
    </Page>
  );
...
function ProfilePage() {
  ...
  useEffect(()=>{(async()=>{
    const [p,rs] = await Promise.all([db.getP(), db.getR()]);
  ...
  const chooseLevel = v => { localStorage.setItem("bw_user_level",v); setUL(v); };
  ...
  const lc = LVC[userLevel]||C.muted;
  ...
  return (
    <Page>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:44,marginBottom:20}}>
        <div style={{fontSize:26,fontWeight:300}}>Профиль</div>
        {has&&!editing&&(
          <IconBtn onClick={()=>setEditing(true)}><I.Edit size={16}/></IconBtn>
        )}
      </div>

      {/* ═══ VIEW MODE ═══ */}
      {!editing&&<>
        {/* Карточка: фото слева, данные справа */}
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:"18px",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            {/* Фото */}
            <div style={{width:72,height:72,borderRadius:"50%",flexShrink:0,overflow:"hidden",
              background:"linear-gradient(135deg,#4ade80,#22d3ee)",
              display:"flex",alignItems:"center",justifyContent:"center",
              boxShadow:"0 0 20px rgba(74,222,128,0.2)",position:"relative"}}>
              {photoUrl&&(
                <img src={photoUrl} referrerPolicy="no-referrer" alt=""
                  style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}}
                  onError={e=>e.target.style.display="none"}/>
              )}
              <div style={{fontSize:22,fontWeight:700,color:"#000",position:"relative",zIndex:photoUrl?-1:1}}>
                {initials}
              </div>
            </div>
            {/* Данные */}
            <div style={{flex:1,minWidth:0}}>
              {/* Имя */}
              <div style={{fontSize:20,fontWeight:600,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>
                {displayName||"—"}
              </div>
              {/* Возраст */}
              {profile.age&&(
                <div style={{fontSize:14,color:C.sub,marginTop:3}}>{profile.age} лет</div>
              )}
              {/* Вес и уровень */}
              <div style={{display:"flex",gap:8,marginTop:6,flexWrap:"wrap",alignItems:"center"}}>
                {profile.weight&&(
                  <span style={{fontSize:13,color:C.sub}}>{profile.weight} кг</span>
                )}
                {userLevel&&(
                  <span style={{fontSize:12,fontWeight:700,color:lc,background:`${lc}18`,borderRadius:6,padding:"2px 9px"}}>
                    {LVL_LABEL[userLevel]}
                  </span>
                )}
              </div>
          ...
          <button onClick={()=>setLvlSheet(true)}
            style={{width:"100%",marginTop:14,background:`${lc}0e`,border:`1px solid ${lc}20`,borderRadius:12,
              padding:"10px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:18}}>{({beginner:"🌱",intermediate:"⚡",pro:"🏆"})[userLevel]||"🎯"}</span>
              <span style={{fontSize:14,fontWeight:600,color:userLevel?lc:C.muted}}>
                {userLevel ? LVL_LABEL[userLevel] : "Выбрать уровень"}
              </span>
            </div>
            <span style={{fontSize:12,color:C.muted}}>изменить ›</span>
          </button>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          {[
            {label:"Минут всего",val:totalMins,  color:C.green, max:300},
            {label:"Тренировок", val:totalCount, color:C.cyan,  max:30},
          ].map((s,i)=>(
            <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px"}}>
              <div style={{fontSize:11,color:C.muted,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:6}}>{s.label}</div>
              <div style={{fontSize:30,fontWeight:200,color:s.color}}>{s.val}</div>
              <div style={{height:3,background:"rgba(255,255,255,0.07)",borderRadius:2,marginTop:10,overflow:"hidden"}}>
                <div style={{height:"100%",width:`${Math.min(100,(s.val/s.max)*100)}%`,background:s.color,borderRadius:2,transition:"width 0.5s"}}/>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"16px",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
            <div style={{fontSize:15,fontWeight:600}}>Активность</div>
            <div style={{fontSize:12,color:C.muted}}>эта неделя</div>
          </div>
          <WeekChart results={history}/>
        </div>

        {/* История */}
        {history.length>0&&(
          <div>
            <div style={{fontSize:15,fontWeight:600,marginBottom:12}}>История тренировок</div>
            {history.slice(0,5).map(r=>(
              <div key={r.id} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,
                padding:"12px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:36,height:36,borderRadius:10,
                  background:r.completedIntervals===r.totalIntervals?"rgba(74,222,128,0.12)":"rgba(251,146,60,0.12)",
                  display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:17}}>
                  {r.completedIntervals===r.totalIntervals?"✅":"⏱"}
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:14,fontWeight:500,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>{r.workoutName}</div>
                  <div style={{fontSize:12,color:C.sub,marginTop:2}}>{fmtD(r.totalDuration)}</div>
                </div>
                <div style={{fontSize:11,color:C.muted,flexShrink:0}}>
                  {new Date(r.completedAt).toLocaleDateString("ru-RU",{day:"numeric",month:"short"})}
                </div>
              </div>
            ))}
          </div>
        ...
        <>
          {[
            ...
            <div key={f.k} style={{marginBottom:14}}>
              <div style={{fontSize:12,color:C.sub,letterSpacing:"0.07em",textTransform:"uppercase",marginBottom:7}}>{f.l}</div>
              <div style={{position:"relative"}}>
                <input value={profile[f.k]||""} type={f.type}
                  ...
                  style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${C.border}`,
                    ...
                    padding:`13px ${f.s?"40px":16}px 13px 16px`,outline:"none",boxSizing:"border-box"}}/>
            ...
            {has&&(
              <button onClick={()=>setEditing(false)}
                style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,
                  padding:"14px",color:C.text,fontSize:15,fontWeight:600,cursor:"pointer"}}>
                Отмена
              </button>
            )}
      ...
      {lvlSheet&&<LevelSheet current={userLevel} onSelect={chooseLevel} onClose={()=>setLvlSheet(false)}/>}
    </Page>
  );
}
...
function CreatePage({navigate, editId}) {
  ...
  useEffect(()=>{(async()=>{
    if(editId){
      ...
      if(w){setName(w.name);setIV(w.intervals?.map(iv=>({...iv,t:iv.t||iv.type,d:iv.d||iv.duration}))||[]);}
    } else {
  ...
  return (
    <Page>
      <div style={{display:"flex",alignItems:"center",gap:10,height:44,marginBottom:22}}>
        <IconBtn onClick={()=>navigate("pop")} style={{flexShrink:0}}><I.Back size={18}/></IconBtn>
        <div style={{fontSize:20,fontWeight:600,flex:1}}>{editId?"Редактировать":"Создать тренировку"}</div>
      </div>

      <div style={{marginBottom:18}}>
        <div style={{fontSize:12,color:C.sub,letterSpacing:"0.07em",textTransform:"uppercase",marginBottom:7}}>Название</div>
        <input value={name} onChange={e=>setName(e.target.value)} placeholder="Моя тренировка"
          style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${C.border}`,
            borderRadius:12,color:"#fff",fontSize:16,padding:"14px 16px",outline:"none",boxSizing:"border-box"}}/>
      </div>

      <div style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{fontSize:12,color:C.sub,letterSpacing:"0.07em",textTransform:"uppercase"}}>Интервалы</div>
        ...
        <div style={{fontSize:11,color:C.muted,marginBottom:12}}>Зажмите № и тащите · Свайп влево — удалить · Нажмите время — изменить</div>
        <SortList intervals={intervals} onChange={handleChange}/>
        <button onClick={()=>setIV(p=>[...p,{id:uid(),t:"medium",type:"medium",d:60,duration:60}])}
          style={{width:"100%",background:"transparent",border:`2px dashed ${C.border}`,borderRadius:12,
            color:C.muted,fontSize:14,padding:"14px",cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",gap:6,transition:"border-color 0.2s,color 0.2s"}}
...
function ActivePage({navigate, workoutId}) {
  ...
  const ivStartTs   = useRef(null);
  const ivInitDur   = useRef(0);
  const wStartTs    = useRef(null);
    ...
    const tick=()=>{
      ...
      if(rem<=0){
        ...
        if(next<woRef.current.intervals.length){
          ivRef.current=next; ivStartTs.current=Date.now(); ivInitDur.current=woRef.current.intervals[next].d; setIdx(next);
        } else {
  ...
  const stop=async()=>{
    ...
    if(phase==="ready"){navigate("pop");return;}
  ...
  return (
    <div style={{height:"100vh",background:"linear-gradient(170deg,#08080f 0%,#0d1117 60%,#060608 100%)",
      color:"#fff",display:"flex",flexDirection:"column",paddingTop:SAFE_TOP,boxSizing:"border-box",overflow:"hidden"}}>
          ...
          :<div style={{display:"flex",flexDirection:"column",alignItems:"center",width:"100%"}}>
            <div style={{position:"relative",width:268,height:268,flexShrink:0}}>
              <svg width={268} height={268} style={{transform:"rotate(-90deg)"}}>
                <circle cx={134} cy={134} r={R} fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth={18}/>
                <circle cx={134} cy={134} r={R} fill="none" stroke={col} strokeWidth={18} strokeLinecap="round"
            ...
            <div style={{display:"flex",gap:10,marginTop:16,width:"100%",maxWidth:268}}>
              {[
                ...
                <div key={i} style={{flex:1,background:s.accent?`${cfg.color}12`:"rgba(255,255,255,0.05)",
                  border:s.accent?`1px solid ${cfg.color}25`:"none",borderRadius:12,padding:"9px 8px",textAlign:"center"}}>
            ...
            <div style={{height:40,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",marginTop:10}}>
              {phase==="paused"
                ...
                :next
                  ?<div style={{textAlign:"center"}}>
                    <div style={{fontSize:9,color:C.muted,marginBottom:2,letterSpacing:"0.1em"}}>СЛЕДУЮЩИЙ</div>
                    <div style={{fontSize:13,color:nc?.color||"#fff",fontWeight:500}}>{nc?.emoji} {nc?.label} — {fmtT(next.d)}</div>
                  </div>
                  :<div style={{textAlign:"center"}}>
                    <div style={{fontSize:9,color:C.muted,marginBottom:2,letterSpacing:"0.1em"}}>ДАЛЬШЕ</div>
                    <div style={{fontSize:13,color:C.green,fontWeight:500}}>🏁 Финиш!</div>
                  </div>
              }
            </div>
            ...
            <div style={{display:"flex",gap:3,marginTop:8,alignItems:"center",flexWrap:"wrap",justifyContent:"center",maxWidth:250}}>
              {workout.intervals.map((_,i)=>(
                <div key={i} style={{height:4,width:i===ivIdx?18:i<ivIdx?10:7,borderRadius:2,
                  background:i<ivIdx?IV[workout.intervals[i].t]?.color||C.green:i===ivIdx?"#fff":"rgba(255,255,255,0.1)",
      ...
      {/* Controls */}
      {phase!=="countdown"&&(
        <div style={{flexShrink:0,padding:"14px 20px",paddingBottom:`max(20px, ${SAFE_BOTTOM})`,
          display:"flex",alignItems:"center",justifyContent:"center",gap:24}}>
          <button onClick={stop} {...sph} style={{...sps,width:52,height:52,borderRadius:"50%",
            background:"rgba(255,255,255,0.07)",border:`1px solid rgba(255,255,255,0.1)`,
            display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#fff"}}>
            <I.Stop size={19}/>
          </button>
          <button onClick={playPause} {...pph} style={{...pps,width:78,height:78,borderRadius:"50%",
            background:`linear-gradient(135deg,${cfg.color},${cfg.color}bb)`,border:"none",
            display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",
            boxShadow:`0 0 36px ${cfg.glow}`,color:"#000"}}>
...
function ResultsPage({navigate, resultId}) {
  ...
  useEffect(()=>{ db.getRById(resultId).then(r=>{ if(!r) navigate("pop"); else setR(r); }); },[resultId]);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 20px"}}>
      <div style={{width:100,height:100,borderRadius:"50%",
        background:done?"linear-gradient(135deg,#4ade80,#22d3ee)":"linear-gradient(135deg,#fb923c,#f43f5e)",
        display:"flex",alignItems:"center",justifyContent:"center",
        boxShadow:done?"0 0 48px rgba(74,222,128,0.35)":"0 0 48px rgba(251,146,60,0.35)",
        marginBottom:20,animation:"scaleIn 0.45s cubic-bezier(0.34,1.56,0.64,1)"}}>
        {done?<I.Trophy size={46} style={{color:"#000"}}/>:<I.Target size={40} style={{color:"#fff"}}/>}
      </div>
      <div style={{fontSize:28,fontWeight:300,marginBottom:6,textAlign:"center"}}>{done?"Отличная работа!":"Тренировка завершена"}</div>
      <div style={{color:C.sub,marginBottom:28,textAlign:"center",fontSize:14}}>{done?"Все интервалы выполнены":"Результат сохранён"}</div>
      ...
      <div style={{width:"100%",maxWidth:380,display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
        {[
          ...
          {icon:<I.Clock size={18} style={{color:C.cyan}}/>,                  bg:C.surface,              brd:C.border,               label:"Время",        val:fmtD(result.totalDuration),                       vc:"#fff"},
          {icon:<I.Target size={18} style={{color:"#a78bfa"}}/>,              bg:C.surface,              brd:C.border,               label:"Интервалы",    val:`${result.completedIntervals} / ${result.totalIntervals}`, vc:"#fff"},
        ].map((s,i)=>(
          <div key={i} style={{background:s.bg,border:`1px solid ${s.brd}`,borderRadius:14,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,animation:`slideUp 0.3s ${0.1+i*0.07}s ease both`}}>
      ...
      <GreenBtn onClick={()=>navigate("pop")} style={{maxWidth:380,...ps}} {...ph}>На главную</GreenBtn>
    </div>
  );
}
...
function DetailsPage({navigate, resultId}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,paddingTop:SAFE_TOP,
      paddingLeft:16,paddingRight:16,paddingBottom:`calc(${NAV_H}px + ${SAFE_BOTTOM} + 16px)`,boxSizing:"border-box",animation:"pageIn 0.2s ease"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,height:44,marginBottom:20}}>
        <IconBtn onClick={()=>navigate("pop")} style={{flexShrink:0}}><I.Back size={18}/></IconBtn>
        <div style={{fontSize:19,fontWeight:600}}>Детали тренировки</div>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {[
          ...
          <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:13,padding:"13px 14px",display:"flex",gap:12}}>
      ...
      <GreenBtn onClick={()=>navigate("pop")} style={{marginTop:20,...ps}} {...ph}>Назад</GreenBtn>
    </div>
  );
}
...
export default function App() {
  const [tab,setTab]     = useState(()=>{ const s=loadTimer(); return s?"start":"start"; });
  const [screen,setScreen] = useState(()=>{ const s=loadTimer(); return s?{type:"workout",id:s.workoutId}:null; });

  const navigate = (type, id=null) => {
    if (type==="pop") { setScreen(null); return; }
    setScreen({type,id});
    window.scrollTo(0,0);
  };
  ...
  // Если есть активный экран поверх основного (тренировка, детали, создание)
  if (screen) {
    if (screen.type==="workout") return <ActivePage  navigate={navigate} workoutId={screen.id}/>;
    if (screen.type==="results") return <ResultsPage navigate={navigate} resultId={screen.id}/>;
    if (screen.type==="details") return <DetailsPage navigate={navigate} resultId={screen.id}/>;
    if (screen.type==="create")  return <CreatePage  navigate={navigate} editId={null}/>;
    if (screen.type==="edit")    return <CreatePage  navigate={navigate} editId={screen.id}/>;
  }

  return (
    <div style={{position:"relative"}}>
      {/* Основные вкладки */}
      {tab==="start"    && <StartPage    navigate={navigate}/>}
      {tab==="programs" && <ProgramsPage navigate={navigate}/>}
      {tab==="profile"  && <ProfilePage  navigate={navigate}/>}
